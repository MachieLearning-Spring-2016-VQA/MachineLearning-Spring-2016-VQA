...
--encoder: RNN body
encoder_net_q=LSTM.lstm_conventional(embedding_size_q,lstm_size_q,dummy_output_size,nlstm_layers_q,0.8)

--MULTIMODAL
--multimodal way of combining different spaces
multimodal_net=nn.Sequential()
				:add(netdef.AxB(2*lstm_size_q*nlstm_layers_q,nhimage,common_embedding_size,0.8))
				:add(nn.Dropout(0.8))
...
for iter = 1, opt.max_iters do
	...
	if iter <= 30000 then
		...
		optim.adam(JdJ, optimize.winit, optimize, state)
	end
