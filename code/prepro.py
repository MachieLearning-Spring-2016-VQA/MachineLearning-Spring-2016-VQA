...
def main(params):
	...
	# create subset for crossing validation
	mycounter = 0
	myloops = params['num_cv']
	unique_img_cv = []
	img_pos_cv = []
	while myloop <= myloops:
		myid = random.randint(1, 15643)
			unique_img_cv.append(unique_img_train[myid])
			unique_img_train.pop([myid])
			img_pos_cv.append(img_pos_train[myid])
			img_pos_train.pop([myid])
	
    # get the answer encoding.
    A = encode_answer(unique_img_train, atoi)
    B = encode_answer(unique_img_cv, atoi)
    ...
    f.create_dataset("vc_answers", dtype='uint32', data=B)
    ...
    f.create_dataset("img_pos_vc", dtype='uint32', data=img_pos_vs)
    ...
    out['unique_img_vc'] = unique_img_vc
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_cv', required=True, type=int, help='number of images for cross validation.')
